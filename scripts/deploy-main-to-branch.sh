...
remote_main_sha="$(git -C "$PROJECT_ROOT" rev-parse origin/main)"
...
git -C "$PROJECT_ROOT" rev-parse "origin/$target"
...
echo "branch SHA: $(git -C "$PROJECT_ROOT" rev-parse origin/$target)"
